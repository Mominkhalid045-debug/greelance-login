...
$final2 = @(
    @{ file = "media__1785482913525.png"; slug = "machine_learning"; name = "Machine Learning" },
    @{ file = "media__1785482933202.png"; slug = "autonomous_systems"; name = "Autonomous Systems" }
)

foreach ($item in $final2) {
    ...
    if (Test-Path $filePath) {
        ...
        if ($found -and ($maxX - $minX) -ge 4) {
            ...
            $cropped.Dispose()
            Write-Host "UPDATED FINAL 2 ICON: $($item.name) -> $outName (${cropW}x${cropH} at X:$cropX Y:$cropY)"
        } else {
