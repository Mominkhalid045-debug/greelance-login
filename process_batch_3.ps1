...
$batch3 = @(
    @{ file = "media__1785482295312.png"; slug = "fintech"; name = "Fintech" },
    @{ file = "media__1785482295318.png"; slug = "cryptocurrency"; name = "Cryptocurrency" },
    @{ file = "media__1785482295330.png"; slug = "systems_engineering"; name = "Systems Engineering" },
    @{ file = "media__1785482295339.png"; slug = "virtual_augmented"; name = "Virtual/Augmented" },
    @{ file = "media__1785482295368.png"; slug = "robotics"; name = "Robotics" }
)

foreach ($item in $batch3) {
    ...
    if (Test-Path $filePath) {
        ...
        if ($found -and ($maxX - $minX) -ge 4) {
            ...
            $cropped.Dispose()
            Write-Host "UPDATED BATCH 3 ICON: $($item.name) -> $outName (${cropW}x${cropH} at X:$cropX Y:$cropY)"
        } else {
